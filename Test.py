...
@hydra.main(version_base = "1.2", config_path = "configs" , config_name = 'config_test') #Will need to be at least version 1.2 for Hydra sweeping
def main(cfg: DictConfig):
    ...
    shutil.copy(cfg.paths_.trained_model, checkpoint_copy)
    shutil.copy(cfg.paths_.copy_configs, config_copy)


    # prepare modalities paths from configs for data loader
    test_modalities = {
        name: (paths.before, paths.after)
        for name, paths in cfg.paths_.modalities.items()}

    # Load in desired data using test loader
    test_data = TestLoader(
        modalities = test_modalities,
        label_dir= cfg.paths_.label_dir,
        patch_size = cfg.model.patch_size, 
        stride = cfg.model.stride,
        add_random_offset = cfg.model.add_random_offset)
    #torch batch loader
    test_dataloader = DataLoader(test_data, num_workers = cfg.model.num_workers)

    # load in decoder + encoder based on trained model path provided
    checkpoint_path = cfg.paths_.trained_model
    ...
    with torch.no_grad():
        for x , y, (i, coord_y, coord_x), (pad_left, pad_right, pad_top, pad_bottom) in test_dataloader:
